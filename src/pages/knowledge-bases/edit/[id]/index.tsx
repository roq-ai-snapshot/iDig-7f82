...
import { getKnowledgeBaseById, updateKnowledgeBaseById } from 'apiSdk/knowledge-bases';
...
function KnowledgeBaseEditPage() {
  ...
  const handleSubmit = async (values: KnowledgeBaseInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateKnowledgeBaseById(id, values);
      mutate(updated);
      resetForm();
    } catch (error) {
      setFormError(error);
